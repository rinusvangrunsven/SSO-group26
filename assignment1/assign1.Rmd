---
title: "Assignment 1"
author: "Rinus van Grunsven (10755373), Florens Douwes (11254483), Imad Lotfi (14651610)"
...
**b)** 
The length is: `r length(data_read_1$birthweight)`:
```{r}
n = length(data_read_1$birthweight)
n
```

The standard deviation is: `r format(sd(data_read_1$birthweight), digits = 2)`:
```{r}
s = round(sd(data_read_1$birthweight), 3)
s
```

The test statistic is: `r qt(0.95,df=length(data_read_1$birthweight)-1)`:
```{r}
...
$$
where $p$ = population proportion, $\hat p$ = sample proportionn $n$ = number of sample values, $E$ = margin of error, and $z_{a/2}$ is the z-score.
...
n must satisfy the following formula, which we can rewrite:
$$
...
We also ran a second test in order to confirm our conclusion stated above. For this second test we only took one novel of Austen and compared this with the one from the admirer. A new matrix was made for this, see below.
