...
par(mfrow=c(3,3));qqnorm(df$diameter, main="QQ-plot diameter");qqnorm(df$volume, main="QQ-plot volume");qqnorm(df$height, main="QQ-plot height");hist(df$diameter, main="Histogram diameter");hist(df$volume, main="Histogram volume");hist(df$height, main="Histogram height");boxplot(df$diameter, main="Boxplot diameter");boxplot(df$volume, main="Boxplot volume");boxplot(df$height, main="Boxplot height")
```
The data follows a fairly normal distribution, as far as we can know with the limited dataset.

#```{r}
#plot(df$diameter, df$height,col=factor(df$type,labels=c("blue","red")))
#```
...
```
For both tree types, the volume and diameter show a clear relation (straight line). This could suggest a relation. The relation also seems present in both tree types.
